...
GregoryPatchComponent::GregoryPatchComponent(const GregoryPatchData& data, mge::Entity& gregory_entity,
                                             mge::Entity& vectors_entity)
    : m_data(data), m_self(gregory_entity), m_vectors(vectors_entity) {
...
unsigned int GregoryPatchComponent::get_line_count() const { return m_line_count; }

void GregoryPatchComponent::set_line_count(unsigned int line_count) { m_line_count = line_count; }
...
void GregoryPatchComponent::update_position() {
  ...
  m_self.patch<mge::TransformComponent>([&center](auto& transform) { transform.set_position(center); });
}
...
std::vector<GeometryVertex> GregoryPatchComponent::generate_vectors_geometry() const {
  ...
  for (int i = 0; i < m_data.patch_sides.size(); ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      vectors.emplace_back(m_data.p2[side]);
      vectors.emplace_back(m_data.fi1[side][j]);

      vectors.emplace_back(m_data.p1[side]);
      vectors.emplace_back(m_data.fi2[side][j]);
    }
  }
